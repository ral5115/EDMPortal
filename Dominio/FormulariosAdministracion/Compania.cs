...
namespace Dominio.FormulariosAdministracion
{
    public class Compania
    {
        ...
        Infraestructura.CRUD.BDCompania _BDCompania = new Infraestructura.CRUD.BDCompania();

        public DataSet Consultar()
        {
            return _BDConsultar.Query("Sp_ListadoCompania");
        }

        public void Actualizar(ModeloCompania Modelo)
        {
            _BDCompania.Sp_ActualizarCompania(Modelo.RowId, Modelo.DC_Nit, Modelo.DC_RazonSocial, Modelo.DC_NombreUsuario,
                ...
                Modelo.EP_ColorBotonEliminar, Modelo.EP_ColorBotonRegresar, Modelo.EP_UrlFondo == null ? string.Empty : Modelo.EP_UrlFondo);
        }

        public ModeloCompania EnviarModelo(int RowId)
        {
            DataSet DsGenerico = _BDCompania.Sp_ConsultarCompania(RowId);
